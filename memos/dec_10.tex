...
\item If so, to what extent is this an ex post reaction to declines in hiring/wages vs. \textit{ex ante expectations} about future automation risk?
\begin{itemize}
...
\section{National Student Clearinghouse (NSC)}
This is the data I use on college major enrollments. It is self-reported by colleges representing 97\% of enrollment. Colleges participate in NSC voluntarily to save administrative costs by outsourcing thousands of enrollment and degree verification requests to loan servicers, employers, and other entities. Accuracy is incentivized because errors create immediate operational problems (students incorrectly placed in loan repayment, failed background checks) that generate complaints and manual work, eliminating the cost savings.\\


 The publicly available data are reported 45 days after start and finish of school semesters, at the state-major level. For all of my analyses, I am aggregating to the major level. 

% \subsection{Micro Data}
% I am not  sure that I need this, although literally aggregating to the year-level, like I am doing now seems too coarse; we're talking basically 5 datapoints (for each major). Seeing it at the institution level would be cool (E.g. are people just dropping CS majors at good colleges). For about \$1000 they will give you enrollment data for any list of students. But as far as I can tell, there is no prodcut for just ``all students''.

\section{Analyses}
\subsection{Caveat on data}
\subsection{A change in enrollment from $t$ to $t+1$}
One issue with the data currently, is it is a \textbf{stock} of people across all years majoring in each earea. I am primarily interested in \textbf{flows}, though, i.e. people declaring a major in year $t$.

Consider a decline in the total people majoring in Computer Science, I cannot distinguish between the following. 
\begin{itemize}
...
Micro data would allow me to disentangle these, however. This is purchasable for less than \$1000, but I need to define the student target population (and provide names).
% For example, the Texas ERC has this. 
...
%but it's useful to be clear on where variation could come from. 
...
\begin{table}[htbp]
   ...
   \quad (2) & \textbf{Occupation-major linkage}: IPUMS ACS 2013-2017 (5-year estimates) for ages 22-35, linking field of degree to current occupation. We use this vintage to ensure SOC code definitions align with those in Felten et al. (2023).\footnote{For the cases where the SOC is masked due to a small population (e.g. Nurse wives and Nurse Practitioners show up as 2911XX ), we take the median among the unmasked SOCs which that category could belong to in the Felten et al data.}  \\
   ...
   On calculation of $w_{i,c}$:

   Since ACS uses FOD (Field of Degree) codes while enrollment data uses CIP (4-digit Classification of Instructional Programs) codes, I harmonize using a many-to-many NCES crosswalk, and then (ii) weight by empirical enrollment shares in each CIP when an ACS FOD maps to many CIPs. 
   

   For example, if a FOD maps to multiple CIPs, which we denote $\text{CIP}(FOD_i)$, then we split that person using empirical enrollment shares in each of those CIPs, from 2023. \text{PERWT}i is the  person weight from ACS. 
...
The Felten et al measure is \textbf{not} designed to capture automation risk, as they say, ``we remain agnostic to whether occupational exposure to generative AI is likely to result in automation vs. augmentation of occupations''. \\
...
\end{figure}
We see a deceleration in computer science and computational and information science between 2024 and 2025, as reported on in the popular press.

But, other trends, unrelated to generative AI, have also been occuring. For example, the secular decline in Communication majors (high AI exposure) and Criminal Justice (low AI exposure) majors, which has been occuring since at least 2019.  

\subsubsection{Wages by AI exposure}
The most recent wage $\times$ major data which are compatible with the above SOC codes is the 2013-17 ACS data. 

\begin{figure}
   \centering
   \includegraphics[width=\textwidth]{/Users/jeffreyohl/Dropbox/CollegeMajorData/output/wage_vs_aioe_scatter.png}
   \caption{Wage vs. AI Occupational Exposure by Major. Each point represents a 4-digit CIP code. Wages are mean earnings from the 2013-2017 ACS for degree holders ages 22-35. AIOE scores are derived from Felten et al. (2023). Includes OLS regression line with slope, intercept, p-values, and R². Top 5 most populous majors are labeled.}
   \label{fig:wage_aioe_scatter}
\end{figure}


\subsection{Motivating DiDs}
It seems like if anything was happening, it was not happening until 2025. So I run a DiD with just two periods between 2024-5. 
I cut the data into high and low AI exposure majors (above/below median AIOE), and test for differential changes in enrollment levels and enrollment growth rates. 



This specificaiton is not well-motivated by any specific exogenous shock in 2024/2025. Most papers that try to do DiDs like this use the launch of Chat GPT on Nov 2022 as the shock, but it's pretty clear from the raw data we should not expect to see anything there. 

Table \ref{tab:did_2024_2025} presents a simple difference-in-differences comparing 2024-2025 enrollment changes for high vs. low AI exposure majors. Columns (1)-(2) examine level changes (Enrollment$_{2025}$ - Enrollment$_{2024}$), while columns (3)-(4) examine growth acceleration (whether enrollment growth accelerated or decelerated relative to the prior year). Columns (2) and (4) add controls for log mean wages (2013-2017).

. High AI exposure majors show slightly higher level growth (+297-302 students, p $>$ 0.10) but marginally significant growth \textit{acceleration} (+3.8 percentage points, p $<$ 0.10). With only one ``post-period'' and wide standard errors, these results should be interpreted cautiously.

\input{/Users/jeffreyohl/Dropbox/CollegeMajorData/output/did_2024_2025.tex}

Table \ref{tab:did_chatgpt} presents an alternative specification using the launch of ChatGPT (November 2022) as the treatment date. I compare average enrollment in the pre-period (2019-2022) to the post-period (2023-2025) for high vs. low AI exposure majors. While level changes show no significant differential (columns 1-2), growth rates reveal that high AI exposure majors grew 7.8 percentage points \textit{less} than low AI exposure majors (p $<$ 0.01). This result is robust to wage controls and suggests that, over the full post-ChatGPT period, enrollment shifted away from AI-exposed fields.

\input{/Users/jeffreyohl/Dropbox/CollegeMajorData/output/did_chatgpt.tex}
...
\begin{itemize}
\item Two majors; roughly, one should be ``low-risk'', it pays off the same in each state, and one should be high-risk, it pays off little in the state you get automated in, and a lot otherwise.
\item We could think of this as nursing and computer science. 
\item The risk agents face is macro-risk to their profession, not uncertainty about your own type, which are sometimes studied. 
\item Expectations about wages for each major.
...
If demand for employment in each degree is perfectly elastic at wage $w_R(H) > w_S > w_R(L)$, then these wages fully pin down the equilibrium.  This means any split of majors, and in fact, any wages, are an equilibrium as long as Equation \ref{eq:indiff} is satisfied.
...
$$\frac{\partial w_{r,H}}{\partial p_H} = \frac{(a_L - a_H)}{2b} < 0$$
 ...
 This is not a deviation from rational expectations, it is just that as $p_H \to 0$, entering a profession becomes more like a lottery ticket. This is the idea of a shortage of workers entering the risky profession, but wages rising ex post when the risk is not realized, (or - not modeled - if this is realized later than expected).  A running example here is radiologists.  This is the same idea, as in finance, there can be what looks like a ``bubble'' ex post, even with rational expectations.


\section{Promise of this idea}

\begin{itemize}
\item   I find this question interesting, but I am likely racing against other people\\ 
\item We are early for this to be studied empirically. The bend on the CS major curve seems to be a 2024-25 phenomenon. The current semester's data will be released in January 2026
\item Is this an interesting question if there is a null result? It seems more interesting if AI exposed majors were falling off a cliff, as that would be evidence people think this technology will impact employment and wages in the professions of that major. Though, perhaps a null would be indicative that college students' beliefs are that AI won't really automate computer sciene jobs. 
...
\subsection{National Survey of College Graduates}
